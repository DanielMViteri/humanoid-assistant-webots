...
def main() -> int:
    ...
    available = [s["stage"] for s in data["latency"]["segments"] if s["status"] == "Available"]
    print(f"\nComputable segments now: {available or 'none'}")
    ...
    if available:
        print("\nPASS: at least one latency segment computes a real value from live Atlas data.")
        return 0
    ...
    print("  3) let the importer run (webots_event_importer --follow)")
    print("  4) open/refresh the provider telemetry once, then re-run this script")
    return 0
